<template>
  <MobileShell :screen="screen" @reach-bottom="onReachBottom">
    <template v-if="screen==='followupHistory'">
      ...
      <PageState :loading="detailState.loading.value" :error="detailState.error.value" :empty="!followups.length" :has-data="!!followups.length" empty-text="暂无回访记录" @retry="loadHistory"><view v-for="f in followups" :key="f.id" class="card followup-card"><AuthImage v-if="f.images?.[0]?.url" class="followup-cover" :src="f.images[0].url" mode="aspectFill"/><text class="card-title">回访 #{{f.id}}</text><text>{{f.content||f.healthCondition||'仅上传图片'}}</text><text class="muted">{{f.createdAt}}</text><button class="btn" @click="goto('followupDetail',{id:f.id})">查看详情</button></view></PageState>
    </template>
    ...
    <template v-else-if="screen==='rescuerFollowups'">
      <BackButton/><view class="title">负责动物回访 · 救助人员只读</view><PageState :loading="rescuer.loading.value" :error="rescuer.error.value" :empty="!rescuer.records.value.length" :has-data="!!rescuer.records.value.length" :loading-more="rescuer.loadingMore.value" :has-more="rescuer.hasMore.value" @retry="rescuer.retry"><view v-for="f in rescuer.records.value" :key="f.id" class="card followup-card"><AuthImage v-if="f.images?.[0]?.url" class="followup-cover" :src="f.images[0].url" mode="aspectFill"/><text class="card-title">回访 #{{f.id}}</text><text>{{f.content||f.healthCondition||'仅上传图片'}}</text><button class="btn" @click="goto('followupDetail',{id:f.id})">查看回访详情</button></view></PageState>
    </template>
...
<style scoped>.followup-card{overflow:hidden}.followup-cover{display:block;width:calc(100% + 48rpx);height:320rpx;margin:-24rpx -24rpx 24rpx}</style>
<script setup>
