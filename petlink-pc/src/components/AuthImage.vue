<template><img :src="resolved||fallback" :alt="alt" loading="lazy" decoding="async" v-bind="$attrs" @error="onError"/></template>
<script setup>
...
function onError(){if(props.fallback&&resolved.value!==props.fallback)resolved.value=props.fallback}
