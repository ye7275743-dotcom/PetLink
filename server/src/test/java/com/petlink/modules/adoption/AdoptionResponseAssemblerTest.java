...
class AdoptionResponseAssemblerTest {
    @Test void recordPageLoadsAnimalsAndCoversInTwoBatchQueries(){
        ...
        AdoptionResponseAssembler assembler=new AdoptionResponseAssembler(animals,images,records,access);
        Animal first=animal();Animal second=animal();second.setId(5002L);second.setName("Coco");
        AdoptionRecord firstRecord=record(7001L,5001L);AdoptionRecord secondRecord=record(7002L,5002L);
        AnimalImage cover=new AnimalImage();cover.setId(5101L);cover.setAnimalId(5001L);
        when(animals.selectBatchIds(java.util.Set.of(5001L,5002L))).thenReturn(List.of(first,second));
        when(images.selectCovers(List.of(5001L,5002L))).thenReturn(List.of(cover));
        var out=assembler.records(List.of(firstRecord,secondRecord));
        assertEquals(2,out.size());assertEquals("/api/media/animal-images/5101",out.get(0).getAnimal().getCoverImageUrl());assertNull(out.get(1).getAnimal().getCoverImageUrl());
        verify(animals,never()).selectById(anyLong());verify(images,never()).selectCover(anyLong());
    }
    ...
    private AdoptionRecord record(Long id,Long animalId){AdoptionRecord r=new AdoptionRecord();r.setId(id);r.setApplicationId(6001L);r.setAnimalId(animalId);r.setUserId(1001L);r.setAdoptedAt(LocalDateTime.now());r.setCreatedAt(LocalDateTime.now());return r;}
